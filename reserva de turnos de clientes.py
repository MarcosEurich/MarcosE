...
def load_css():
    st.markdown("""
    <style>
        .stButton>button { width: 100%; border-radius: 8px; padding: 0.8em 0.2em; }
        ...
        .calendar-grid-container .stHorizontalBlock { display: flex; flex-wrap: wrap; justify-content: space-between; }
        .calendar-grid-container .stButton { flex-basis: 13%; max-width: 13%; padding: 2px; }
    </style>
...
def display_client_view():
    """Muestra la interfaz para que el cliente reserve un turno."""
        ...
        with st.container(border=True):
            ...
            with st.form("appointment_form"):
                ...
                if st.session_state.selected_date:
                    booked_slots = [ appt['timeSlots'] for appt in st.session_state.data['appointments'] if appt['date'] == st.session_state.selected_date.strftime('%Y-%m-%d')]
                    booked_slots = [slot for sublist in booked_slots for slot in sublist] # Aplanar lista
                ...
                selected_time_slots = st.multiselect(
                    ...
                    placeholder="Seleccione un día del calendario primero"
                )
                if st.session_state.selected_date:
...
def display_admin_view():
    """Muestra el panel de administración con todas sus funcionalidades."""
    ...
    with tab3:
        # Lógica para acciones como 'Día de Lluvia'...
